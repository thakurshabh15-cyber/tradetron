...
async def get_historical_candles(
    ...
    return {
        ...
        "timeframe": timeframe,
        "count": len(candles),
        "source": getattr(unified_market_manager, "last_candle_source", "REAL") or "REAL",
        "candles": candles,
    }
...
async def get_providers_status():
    ...
    return {
        ...
        "disclaimer": (
            ...
            "Real broker API keys stream live ticks. Candle responses include a "
            "'source' field (REAL vs SIMULATED) so clients never confuse "
            "synthetic candles with real exchange data."
        ),
    }
...
def _spot_from_quote(quote: Any) -> Optional[float]:
