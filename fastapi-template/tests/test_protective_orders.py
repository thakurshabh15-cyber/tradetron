...
async def test_failed_row_with_live_ref_not_re_poked_by_reconcile(monkeypatch):
    # A FAILED row with a broker reference means the old order may STILL be live
    # at the broker.  Reconcile re-arm is gated on a successful cancel first —
    # when the broker reject the cancel, no re-placement may happen (that would
    # risk duplicate protection against real inventory).
    broker = _CancelFailingBroker()
    monkeypatch.setattr("app.brokers.get_broker_adapter", lambda acc: broker)
    user_id, acc_id = await _seed_user_and_broker()
    pos_id = await _seed_position(
    ...
    for leg in (LEG_STOP_LOSS, LEG_TAKE_PROFIT):
        ref = f"FAKE-STILL-LIVE-{leg}"
        await _seed_protective_row(
            pos_id, user_id, acc_id,
            leg=leg,
            status="FAILED",
        ...
        broker.status_map[ref] = "OPEN"

    await _ENGINE.reconcile_once()

    rows = await _protective_rows(pos_id)
    assert all(r.status == "FAILED" for r in rows)  # never re-poked while live
    assert all(r.broker_protective_order_id for r in rows)  # refs kept visible
    assert all("duplicate protection" in (r.last_error or "").lower() for r in rows)
    assert broker.placed == []  # no replacement leg dispatched
...
async def test_reconcile_recovers_pending_position_with_no_rows(monkeypatch):
    ...
    # `report["recovered"]` holds position_id strings (the reconciled positions
    # that were re-armed outside the scan session).
    assert pos_id in report["recovered"]
    rows = await _protective_rows(pos_id)
    assert {r.leg for r in rows} == {LEG_STOP_LOSS, LEG_TAKE_PROFIT}
    assert all(r.status == "PLACED" for r in rows)
    assert (await _position_state(pos_id)) == PROTECTION_STATE_PROTECTED
